...
class MVXTwoStageDetector(Base3DDetector):
    ...
    def __init__(self,
        ...
        if img_backbone_distill:
            self.img_backbone_distill = builder.build_backbone(img_backbone_distill)
        if img_neck_distill is not None:
            self.img_neck_distill = builder.build_neck(img_neck_distill)
        if img_view_transformer_distill:
            self.img_view_transformer_distill = builder.build_neck(img_view_transformer_distill)
        if img_bev_encoder_backbone_distill:
            self.img_bev_encoder_backbone_distill = builder.build_backbone(img_bev_encoder_backbone_distill)
        ...
        if radar_bbox_head:
            ...
            self.radar_bbox_head = builder.build_head(radar_bbox_head)
