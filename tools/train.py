...
def main():
    ...
    if hasattr(cfg, 'plugin'):
        if cfg.plugin:
            ...
            from projects.mmdet3d_plugin.ssc_rs.apis.train import custom_train_model
    # set cudnn_benchmark
    ...
    if args.work_dir is not None:
        ...
        cfg.work_dir = osp.join('./result/ssc_rs',
    ...
    if cfg.sync_bn:
        logger.info(f'Converted sync bn')
        model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model)

    
    if args.finetune is not None:
        logger.info(f'Model: load pretrained model {args.finetune}')
        ckpt = torch.load(args.finetune)
        model.load_state_dict(ckpt['state_dict'])
    if args.pretrained is not None:
        logger.info(f'Model: load pretrained model {args.pretrained}')
        ...
        for key, val in ckpt['state_dict'].items():
            ...
            if key in s and s[key].shape == val.shape:
                ...
                logger.info('igonre weight from not found key {}'.format(key))
            else:
                logger.info('ignore weight of mistached shape in key {}'.format(key))
