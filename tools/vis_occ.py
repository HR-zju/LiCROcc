...
def main():
    ...
    info_path = \
        args.root_path + '/nuscenes_occ_infos_%s.pkl' % args.version
    ...
    for cnt, info in enumerate(
        ...
        gt_occ_path = os.path.join('data/nuScenes-Occupancy',rel_path)#self.data_infos[index]['occ_path']
        scene_name = [tem for tem in gt_occ_path.split('/') if 'scene_' in tem][0]
        ...
        # gt_data = np.load(os.path.join(args.root_path, gt_occ_path))

        # 换benchmark
        occ_gt_path = gt_occ_path
        pcd = np.load(occ_gt_path)
        occ_label = pcd[..., -1:]
        ...
        target = np.zeros([512, 512, 40], dtype=np.uint8)
