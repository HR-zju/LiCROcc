...
class SemanticBranch(nn.Module):
    ...
    def forward(self, vw_features, coord_ind, full_coord, info):
        ...
        return dict(
            ...
            mss_indexes = [vw1_coord, vw2_coord, vw3_coord] if self.phase == 'trainval' else None,
            mss_logits = [self.out2(proj1_vw), self.out4(proj2_vw), self.out8(proj3_vw)] if self.phase == 'trainval' else None
        )

    def losses(self, out_dict, metas):
        mss_logits = out_dict['mss_logits']
        mss_indexes = out_dict['mss_indexes']
        device = mss_logits[0].device

        class_weights = torch.from_numpy(1 / np.log(np.array(self.class_frequencies) + 0.001)).float().to(device)
        loss_dict = {}
