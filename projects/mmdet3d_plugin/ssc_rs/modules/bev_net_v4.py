...
class TemporalGuidedModule(nn.Module):
    def __init__(self, channel):
        ...
        self.conv_out = nn.Conv2d(channel, channel, kernel_size=3, padding=1)
    ...
    def forward(self, xt, xt_1):
        ...
        xt = xt + self.conv_out(xt_star)

        return xt


@HEADS.register_module()
class BEVUNetV4(nn.Module):
    def __init__(self, n_class, n_height, class_frequences=None, dilation=1, 
        ...
        self.guidance_conv = outconv(128 if not light else 64, n_height*24)
    ...
    def forward(self, x, sem_fea_list, com_fea_list, return_bev=False, prev_bev=None):
        ...
        assert xg.shape[0] == 1
        xg = xg.view(xg.shape[0], 24, self.n_height, *xg.shape[-2:])
        xg = xg.permute(0,1,4,3,2)
