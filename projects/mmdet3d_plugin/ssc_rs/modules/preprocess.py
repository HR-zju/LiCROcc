...
class PcPreprocessor(nn.Module):
    def __init__(self, lims, sizes, grid_meters, pc_dim=10,init_size=32, offset=0.5, pooling_scales=[0.5, 1, 2, 4, 6, 8], frozen=False):
        # todo move to cfg
        super().__init__()
    ...
    def add_pcmean_and_gridmean(self, pc, bxyz_indx, return_mean=False):
        _, value = torch.unique(bxyz_indx, return_inverse=True, dim=0)
        pc_mean = torch_scatter.scatter_mean(pc[:, :3], value, dim=0)[value] # 平均坐标
