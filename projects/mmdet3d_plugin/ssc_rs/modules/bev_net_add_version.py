...
class BEVFusion(nn.Module):
    def __init__(self, channel, light=False, use_cam=[True, True, True], use_DA=False, use_add=False):
        super().__init__()
...
class BEVUNet(nn.Module):
    ...
    def forward(self, x, sem_fea_list, com_fea_list, img_feats_list):
        # breakpoint()
        # 绘制特征图
        # draw_feat(img_feats_list, 'img')
        # draw_feat(sem_fea_list, 'sem')
        # draw_feat(com_fea_list, 'com')
        # breakpoint()

        x1 = self.inc(x)    # [B, 64, 256, 256]
        x2 = self.down1(x1)    # [B, 128, 128, 128]
        x2_f = self.bev_fusions[0](x2, sem_fea_list[0], com_fea_list[0], img_feats_list[0]) # 128, 64, 64 -> 128
        draw_feat(x2, 'x2')
        if self.use_Distill_2:
        ...
        x3_f = self.bev_fusions[1](x3, sem_fea_list[1], com_fea_list[1], img_feats_list[1]) # 256, 128, 128 -> 256
        if self.use_Distill_2:
        ...
        x4_f = self.bev_fusions[2](x4, sem_fea_list[2], com_fea_list[2], img_feats_list[2]) # 512, 256, 256 -> 512
        if self.use_Distill_2:
        ...
        if self.use_Distill_2:
            return x, [x2_f, x3_f, x4_f], [x2_f_residue, x3_f_residue, x4_f_residue]
        else:
            return x, [x2_f, x3_f, x4_f], None
    ...
    def losses(self, scores, labels):
        ...
        # socres torch.Size([4, 17, 512, 512, 40])
        # labels torch.Size([4, 512, 512, 40])
